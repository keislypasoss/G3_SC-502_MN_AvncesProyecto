...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $isAjax) {
    ...
    $puestoActual = $perfilEmpleado['puesto'] ?? '';
...
<head>
    ...
    <title>Soluna | Mi Perfil (admin)</title>
...
<main class="container-fluid d-flex flex-column min-vh-100" style="margin:0;padding:0;">
    ...
    <div class="align-self-center col-11 col-md-6 col-lg-4 m-3 p-2 bg-warning-subtle" style="border-radius: 5px">
        <p id="titulo-perfil" class="h3 text-center text-black">
            Mi Perfil (admin) - <?= e($_SESSION['nombre'] ?? ($perfilEmpleado['nombre'] ?: 'Empleado')) ?>
        </p>
    </div>
    ...
    <div class="container py-4">
        ...
        <form id="form-perfil" method="post" class="row g-3" novalidate>
            ...
            <div class="col-md-6">
                ...
                <input name="nombre" id="nombre" type="text" class="form-control" value="<?= e($perfilEmpleado['nombre']) ?>" required>
            </div>
            ...
            <div class="col-md-12">
                ...
                <input id="puesto" name="puesto" type="text" class="form-control"
                       value="<?= e($perfilEmpleado['puesto']) ?>" readonly>
                <div class="form-text">Este dato lo gestiona RR.HH.</div>
            </div>
...
$(function() {
    ...
    const puestoRegex = /^[\p{L}\d\s.,#()/\-]+$/u;

    function validateFront() {
        const errors = [];
        $('#email,#nombre,#puesto,#password_actual,#password_nueva,#password_confirmar').removeClass('is-invalid');
        ...
        if (puesto) {
            if (puesto.length > 100) {
                errors.push('El puesto no debe exceder 100 caracteres.');
                $('#puesto').addClass('is-invalid');
            } else if (!puestoRegex.test(puesto)) {
                errors.push('El puesto contiene caracteres no permitidos.');
                $('#puesto').addClass('is-invalid');
            }
        }
    ...
    $('#email,#nombre,#puesto,#password_actual,#password_nueva,#password_confirmar').on('input', function(){
        ...
        $.ajax({
            ...
            success: function(data){
                if (data.ok) {
                    ...
                    if (data.nombre) {
                        $('#titulo-perfil').text('Mi Perfil (admin) - ' + data.nombre);
            ...
            error: function(){ showAlert('danger', 'Error de red.'); },
